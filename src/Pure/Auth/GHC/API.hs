...
handleUpdateEmail :: forall _role. Typeable _role => Txt -> Config _role -> MessageHandler (API.UpdateEmail _role)
handleUpdateEmail ip Config { onRecover } = awaiting do
...
handleDelete :: forall _role. Typeable _role => Txt -> Config _role -> MessageHandler (API.Delete _role)
handleDelete ip Config { onDeleted } = awaiting do
